...
- 具体的な検索キーワード（ブランド名+型番 or ブランド名+商品種別の2〜4語）
- 型番を含むキーワードを最優先すること（例: "SONY WH-1000XM4", "Dyson V12"）
- 以下のカテゴリをバランスよくカバー:
...
- 【重要】色名だけのキーワードは不可（「ブラック」「ホワイト」等を単独で使わない）
- 【重要】「中古」「セット」「限定」「動作確認」等の状態・包装を表す語だけの組み合わせは不可
