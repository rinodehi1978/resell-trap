...
# --- Shipping size calculation ---

# サイズ別送料テーブル（3辺合計mm → 転送料円）
_SHIPPING_SIZE_TABLE = [
    (600, 735),
    (800, 840),
    (1000, 960),
    (1200, 1150),
...
SYSTEM_FEE = 100  # 無在庫1配送毎のシステム利用料


def calculate_forwarding_cost(keepa_product: dict) -> int | None:
    """Keepa商品のパッケージサイズから転送料を計算。

    Returns:
        転送料(円): サイズテーブルに収まる場合
        None: サイズデータなし、または200サイズ超で対応不可
    """
    h = keepa_product.get("packageHeight") or 0
    l = keepa_product.get("packageLength") or 0
    w = keepa_product.get("packageWidth") or 0

    if h <= 0 or l <= 0 or w <= 0:
        return None  # サイズデータなし

    total_mm = h + l + w

    for max_mm, cost in _SHIPPING_SIZE_TABLE:
        if total_mm <= max_mm:
            return cost

    return None  # 200サイズ超 → 対応不可
...
class DealCandidate:
    ...
    yahoo_url: str
    yahoo_image_url: str
    yahoo_shipping: int  # Yahoo seller shipping (0 = free, scraped from listing)
    amazon_asin: str
    amazon_title: str
    ...
    sells_well: bool
    # Cost breakdown
    total_cost: int       # yahoo_price + yahoo_shipping + forwarding + system_fee
    forwarding_cost: int  # size-based forwarding cost
    system_fee: int       # system fee per shipment (100 yen)
    amazon_fee: int       # Amazon referral fee (% of sell price)
    # Profit
    sell_price: int           # Amazon sell price (market-based)
    gross_profit: int         # sell_price - total_cost - amazon_fee
...
def score_deal(
    ...
    Gross margin calculation:
      actual_forwarding = size-based cost from Keepa dimensions, or forwarding_cost fallback
    ...
    Returns None if:
      ...
      - Package size exceeds 200 (3-side total > 2000mm)
    """
    ...
    # Used-only strategy: only consider Amazon used price
    if used_price is None or used_price <= 0:
        return None

    # Skip if used price exceeds new price (data anomaly)
    if new_price and new_price > 0 and used_price > new_price:
        return None

    sell_price = used_price

    # Size-based forwarding cost
    size_cost = calculate_forwarding_cost(keepa_product)
    if size_cost is not None:
        actual_forwarding = size_cost
    else:
        # Check if package data exists but exceeds 200-size
        h = keepa_product.get("packageHeight") or 0
        l = keepa_product.get("packageLength") or 0
        w = keepa_product.get("packageWidth") or 0
        if h > 0 and l > 0 and w > 0:
            # Size data present but exceeds table → 200-size超 → 対応不可
            return None
    ...
    # Amazon referral fee
    amazon_fee = int(sell_price * amazon_fee_pct / 100)

    # Gross profit and margin
    gross_profit = sell_price - total_cost - amazon_fee
    gross_margin = (gross_profit / sell_price * 100) if sell_price > 0 else 0.0
    ...
    return DealCandidate(
        ...
        forwarding_cost=actual_forwarding,
